package com.example.darren.borutoapp.util

object Constants {

    const val BASE_URL = "http://10.0.2.2:8080"
    //const val BASE_URL = "https://darren-test-server.herokuapp.com"

    const val DETAILS_ARGUMENT_KEY = "heroId"
    const val HERO_DATABASE_TABLE = "hero_table"
    const val HERO_REMOTE_KEYS_DATABASE_TABLE = "hero_remote_keys_table"
    const val BORUTO_DATABASE = "boruto_database"

    const val DETAILS_ARGUMENT_KEY_GOAT = "goatId"
    const val GOAT_DATABASE_TABLE = "goat_table"
    const val GOAT_REMOTE_KEYS_DATABASE_TABLE = "goat_remote_keys_table"
    const val GOAT_DATABASE = "goat_database"


    const val PREFERENCES_NAME = "boruto_preferences"
    const val PREFERENCES_KEY = "on_boarding_completed"

    const val ON_BOARDING_PAGE_COUNT = 3
    const val LAST_ON_BOARDING_PAGE = 2


    const val ITEMS_PER_PAGE = 3
    const val ABOUT_TEXT_MAX_LINES = 7

    const val MIN_BACKGROUND_IMAGE_HEIGHT = 0.4f


}